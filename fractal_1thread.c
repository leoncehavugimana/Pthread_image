...
typedef struct mandelstruct mandel;
int pal[256] =
{
...
void *mandelbrot(void *mandelbrotptr )
{
    mandel mandelvar;
    mandel* mandelptr;
    mandelptr=( mandel*)mandelbrotptr;
    mandelvar=*mandelptr;
    ...
    float ymin = -1.6f;
    float ymax = 1.6f;
    for (i = 0; i < mandelvar.height; i++)
    {
        for (j = 0; j < mandelvar.width; j++)
        {
            float b = xmin + j * (xmax - xmin) / mandelvar.width;
            float a = ymin + i * (ymax - ymin) / mandelvar.height;
            float sx = 0.0f;
            float sy = 0.0f;
            int ii = 0;
            while (sx + sy <= 64.0f)
            ...
            if (ii == 1500)
            {
                mandelvar.pixmap[j+i*(int)mandelvar.width] = 0;
            }
            else
            {
                int c = (int)((ii / 32.0f) * 256.0f);
                mandelvar.pixmap[j + i *(int)mandelvar.width] = pal[c%256];
            }
        }
    }
    pthread_exit(NULL);
...
{
mandel *mandelmainptr, mandelmain={1024.0f,1024.0f,malloc(1024*1024*sizeof(int))};
mandelmainptr=&mandelmain;
    pthread_t thread;
   int i=pthread_create(&thread,NULL,mandelbrot,mandelmainptr);
    if(i)
    {
    ...
    printf("fractal in process\n");
    writetga(mandelmain.pixmap, 1024, 1024, "fracout.tga");
    free(mandelmain.pixmap);
    pthread_exit(NULL);
}
